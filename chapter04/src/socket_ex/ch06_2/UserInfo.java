...
//			server.getTa().append("[[" + nickName + "]] 입장\n");
//			
//			// 기존 사용자들에게 신규 유저의 접속을 알린다
//			broadCast("NewUser/" + nickName);
//			
//			// 자신에게 기존 사용자들을 알린다
//			for (int i = 0; i < server.getVc_user().size(); i++) {
//				UserInfo userInfo = server.getVc_user().elementAt(i);
//				sendmessage("OldUser/" + userInfo.nickName);
//			}
//			for (int i = 0; i < server.getVc_room().size(); i++) {
//				RoomInfo roomInfo = server.getVc_room().elementAt(i);
//				sendmessage("OldRoom/" + roomInfo.roomName);
//			}
//			
//			// 사용자에게 자신을 알린 후 벡터에 자신을 추가
//			server.getVc_user().add(this);
...
//						server.getTa().append("[[" + nickName + "]]" + msg + "\n");
						//inmessage(msg);
					} catch (IOException e) {
//						try {
//							server.getTa().append(nickName + " : 사용자 접속 끊어짐\n");
//							dos.close();
//							dis.close();
//							user_socket.close();
//							server.getVc_user().remove(this);
...
//			for(int i = 0; i < server.getVc_user().size(); i++) {
//				UserInfo ui = server.getVc_user().elementAt(i);
...
//			for(int i = 0; i < server.getVc_room().size(); i++) {
//				RoomInfo room = server.getVc_room().elementAt(i);
...
//				server.getVc_room().add(new_room);
