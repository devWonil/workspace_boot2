...
public class Client extends JFrame implements ActionListener{
	...
	//UserSocket userSocket = new UserSocket(server, socket);
//	UserInfo userInfo = new UserInfo(socket);
	...
	private void init() {
		setTitle("포트번호는 자동생성됩니다 서버에 포트번호를 넣어주세요");
		...
		img_lbl.setIcon(new ImageIcon("unplug-g360205bb9_640.png"));
		...
		totalList_lbl.setFont(new Font("휴먼모음T", Font.BOLD, 12));
		...
		panel.add(joinRoom_btn);
		hostIP_tf.setText("192.168.4.4"); // 호스트IP
		...
		scrollPane.setBounds(0, 0, 323, 373);
				...
				public void run() {
					while(true) {
						try {
							...
							System.out.println(date +"  서버에서 온 msg : " + msg);
							...
							try {
								...
								dos.close();
								socket.close();
								JOptionPane.showMessageDialog(null, "서버가 종료됨!", "알림", JOptionPane.ERROR_MESSAGE); //경고창
								break;
							} catch (Exception e1) {
								return;
							}
	...
	public void inmessage(String str) {
		...
		if(protocol.equals("NewUser")) {
			...
			my_roomName = message;
			JOptionPane.showMessageDialog(null, "채팅방(" + my_roomName + ")에 입장완료", "알림", JOptionPane.INFORMATION_MESSAGE);
			viewChat_ta.setText("");
